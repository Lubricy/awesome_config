...
if vicious then      
    ...
    -- Graph properties
    netwidget:set_width(40)
    netwidget:set_background_color(theme.bg_normal)
    netwidget:set_color("#0000FF")
    netwidget:set_max_value(10240)
    ...
    vicious.register(netwidget, vicious.widgets.net,
        function (netwidget,args)
            ...
            for k,v in pairs(args) do
                arr = BuildArray(string.gmatch(k,"[^{} ]+"))
                if not (arr[1]=="lo") then
                    nums[arr[2]]=nums[arr[2]] or 0    
                    nums[arr[2]]=nums[arr[2]] +  v    
                end
            end
            return nums["down_b"]
        end
